	...
	: Program("", "")
{
	...
	glslangProgram = CC_NEW(glslang::TProgram)();
	for (auto& m : modules)
	{
	...
	if (!glslangProgram->link(controls))
	{
	...
	CC_ASSERT(defaultDescriptorSetLayout);
...
{
	CC_SAFE_DELETE(_program);
	CC_SAFE_DELETE(glslangProgram);
	CC_SAFE_DELETE(defaultPipelineLayout);
	CC_SAFE_DELETE(defaultDescriptorSetLayout);
}
	...
	for (int i = 0; i < program->getNumLiveUniformBlocks(); ++i)
	{
		...
		for (auto& ty : (*s))
		{
			if (!ty.type->hiddenMember())
			{
				...
				uinfo.size = gfx::GFX_TYPE_SIZES[(int)uniform.type];
	...
	for (auto& block : info.blocks)
	{
		if (blockInfoEx.find(block.name) == blockInfoEx.end())
		{
			...
			for (auto& u : block.members)
			{
				...
				UniformInfo uinfo;
				uinfo.count = u.count;
				uinfo.size = gfx::GFX_TYPE_SIZES[(int)u.type];
		...
		else
		{
			...
			for (auto& u : block.members)
			{
				...
				else
				{
					...
					const auto size = gfx::GFX_TYPE_SIZES[(int)u.type];
...
bool ProgramGFX::compileSpirv()
{
	for (auto& m : modules)
	{
		const auto stage = m->getGFXShaderStage().stage;
		spv::SpvBuildLogger logger;
		glslang::SpvOptions spvOptions;
		std::vector<uint32_t> spirv;
		const auto envStage = UtilsGFX::toGLSLangShaderStage(stage);
		glslang::GlslangToSpv(*glslangProgram->getIntermediate(envStage), spirv, &logger, &spvOptions);
		if (spirv.empty())
		{
...
int ProgramGFX::getAttributeLocation(const std::string& name) const
{
...
UniformLocation ProgramGFX::getUniformLocation(const std::string& uniform) const
{
...
static gfx::Texture* getHandler(TextureBackend* texture)
{
...
static gfx::Sampler* getSampler(TextureBackend* texture)
{
	switch (texture->getTextureType())
	{
	case TextureType::TEXTURE_2D:
	...
	const cocos2d::Map<std::string, BufferGFX*>& buffers_)
{
	...
	if(!program->getHandler())
		return;
	//const auto& infoMap = program->getAllActiveUniformInfo(ShaderStage::VERTEX_AND_FRAGMENT);
...
ProgramStateGFX::~ProgramStateGFX()
{
	...
	CC_SAFE_DELETE(descriptorSet);
}

bool ProgramStateGFX::setUniformBuffer(const std::string& blockName, BufferGFX* buffer)
{
...
BufferGFX* ProgramStateGFX::getUniformBuffer(const std::string& blockName)
{
...
void ProgramStateGFX::bindUniformBuffers(cc::gfx::DescriptorSet* ds, uint32_t set)
{
	if (!program->getHandler())
		return;
	const auto& blocks = program->getShaderInfo().blocks;
...
void ProgramStateGFX::bindUniformTextures(cc::gfx::DescriptorSet* ds, uint32_t set)
{
	if (!program->getHandler())
		return;
	const auto& sts = program->getShaderInfo().samplerTextures;
	for (auto& st : sts)
	{
		...
		for (auto& it : textures.at(st.name))
		{
			...
			if (tex && index < st.count)
			{
				...
				ds->bindTexture(st.binding, getHandler(tex), index);
				ds->bindSampler(st.binding, getSampler(tex), index);
			}
		}
	}
}

void ProgramStateGFX::bindUniformBuffers(uint32_t set)
{
	bindUniformBuffers(descriptorSet, set);
}

void ProgramStateGFX::bindUniformTextures(uint32_t set)
{
	bindUniformTextures(descriptorSet, set);
}
