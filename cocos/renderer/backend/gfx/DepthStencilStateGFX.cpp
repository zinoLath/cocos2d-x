...
DepthStencilStateGFX::DepthStencilStateGFX(const DepthStencilDescriptor& descriptor)
	...
	unsigned int stRefValueBack) const
{
	...
	// depth test
	s.depthTest = _depthStencilInfo.depthTestEnabled ? 1 : 0;
	s.depthWrite = _depthStencilInfo.depthWriteEnabled ? 1 : 0;
